...
C        write(istdo,*)
C        write(istdo,*) '============== ',np(jp),nh(jp),' ============'
C        write(istdo,*) 'i kindx(i), np, nh, scnsty,E, pos/all'
C        write(istdo,*)
C        write(istdo,*) ' Original order'
C        write(istdo,*)
...
!         Orbital jp is supposed to be an ordered one, so should be k
         ...
         IF( nak(ki) .EQ. nakj ) THEN
            ktotal = ktotal + 1
            kindx(ktotal) = ki
C           write(istdo,*) ktotal, ki, np(ki), nh(ki),
C    &                     scnsty(ki) ,E(ki), i
         ENDIF
      ENDDO
...
C        write(istdo,*) j, ki, np(ki), nh(ki), 'fixed orbs'
...
C         IF( j .NE. kfixed + knon) STOP 'orthy: Should not happen'
...
!           non-correlation, using criteria scnsty/E^2 
         ...
         DO i = kfixed + 1, kfixed + knon
            ...
            ENDDO
            kindx(i) = ki
C           write(istdo,*) i, ki, np(ki), nh(ki),scnsty(ki)/E(ki)**2,
C    &                     'non-corre orbs sc/E^2'
         ENDDO
...
!           correlation orbitals, using criteria scnsty
         ...
         DO i = kfixed + knon + 1, ktotal
            ...
            ENDDO
            kindx(i) = ki
C           write(istdo,*) i, ki, np(ki), nh(ki),scnsty(ki),'corre orbs'
         ENDDO
...
! The fixed orbitals are not changed
      ...
      DO Lraw = kfixed + 1, ktotal
         ...
         DO  Kraw = 1, Lraw - 1
            K = kindx(Kraw)
            OVRLAP = RINT (L,K,0)
!            write(istdo,*) 'state',np(k),nh(k),OVRLAP
...
!         DO kraw = 1, Lraw
!            k= kindx(kraw)
!             write(istdo,*) np(L),nh(L),np(k),nh(k),rint(L,k,0)
!         ENDDO
!---------------------------------------------------------------
