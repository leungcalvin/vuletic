...
************************************************************************
*
! Two places where heary calls to fco, gco, and dsubrs are replaced
! by arrays which are obtained by calling new subroutines where
! do-loop over CSF's is performed
!
...
!XHH Add an array to store DSUBRS(EOL,IR,IR) for IR=1,NCF
      LOGICAL imzero
...
! New pointer and array for the vector
      ...
      CALL alloc (pnfgco,ncf,8)
      CALL alloc (pnlocal,ncf,8)
      CALL dsuvec (EOL,NCF)
!XHH The results are now stored in ddsubrs(1:NCF) 
*
*   Initializations
*
      NDIM = 1
...
!               SUMR = SUMR+ddsubrs (IR)*FCO (K,IR,J,IB)
!    1       CONTINUE


             CALL fcovec (K,J,IB,imzero)
            IF (.NOT. imzero) THEN
               DO ir = 1, ncf
                  sumr = sumr + ddsubrs (ir) * FgCO(ir)
               ENDDO
            ENDIF
!XHH 
            IF (IB .EQ. J) THEN
...
!XHH The same as above
!
!               DO 4 IR = 1,NCF
!!                  SUMR = SUMR+DSUBRS (EOL,IR,IR)*GCO (K,IR,J,IB)
!                  SUMR = SUMR+ddsubrs (IR)*GCO (K,IR,J,IB)
!    4          CONTINUE
