************************************************************************
*                                                                      *
      SUBROUTINE fcovec (K,IA,IB,imzero)
!-----------------------------------------------------------------------
!
...
!     for per call to FCO or fcovec since fcovec always calls CLRX
!     once while FCO may not call CLRX in some cases) since fcovec
...
!   _Note_ :
!
!  gcovec.f uses the same common block (/fg2cof/) to store
!  g-coefficients as well
!-----------------------------------------------------------------------
*                                                                      *
...
!                Using DBLE( (I) * ( (I) - 1 ) / 2 )
               FgCO(ir) = DBLE( (IBITS(iiqa(inp1,ir),inp2,8)) *
     &                         ((IBITS(iiqa(inp1,ir),inp2,8))-1)/2)
            ENDDO
