************************************************************************
*                                                                      *
      SUBROUTINE TRSORT (NFILE,LPRINT)
*                                                                      *
...
Cww      INTEGER PNTRIQ
      ...
      CHARACTER*2 NH
*
      POINTER (PIBEG,IBEG(1))
      POINTER (PILAB,ILAB(1))
      POINTER (PILAST,ILAST(1))
      POINTER (PILEFT,ILEFT(1))
      POINTER (PIPTCS,IPTCSF(1))
      POINTER (PIRIGH,IRIGHT(1))
      POINTER (PLBLIN,LBLINT(1))
      POINTER (PIPTR,IPTR(1))
      POINTER (PISLDR,ISLDR(1))
      POINTER (PXSLDR,XSLDR(1))
*
      POINTER (PLABEL,JLABL(1))
      POINTER (PCOEFF,XL(1))
*
...
*
     ...
     :             PIPTCS,PIRIGH,PLBLIN,LLDIM,1)
      CALL ALCNMA (PIPTR,PISLDR,PXSLDR,NMDIM,1)
*
...
*
      M = M+1
      IF (M .GT. NMDIM) CALL ALCNMA (PIPTR,PISLDR,PXSLDR,NMDIM,2)
      I = 1
*
   ...
   20 REWIND (NFILE)
      ...
      IF (NMCP .NE. 0) THEN
         ...
         DO 23 I = 1,NINT
            MUP = IPTCSF(I)
            INTS = MUP-MLR+1
            WRITE (NFILE) LBLINT(I),INTS
            IF (LPRINT) THEN
               IA = LBLINT(I)/KEY
               IB = MOD (LBLINT(I),KEY)
               WRITE (99,302) NP(IA),NH(IA),NP(IB),NH(IB)
            ENDIF
            WRITE (NFILE) (ISLDR(M),XSLDR(M),M = MLR,MUP)
            IF (LPRINT) THEN
               DO 22 M = MLR,MUP
                  IS = MOD (ISLDR(M),NCA)
                  IR = ISLDR(M)/NCA
...
*
     ...
     :             PIPTCS,PIRIGH,PLBLIN,LLDIM,3)
      CALL ALCNMA (PIPTR,PISLDR,PXSLDR,NMDIM,3)
*
      RETURN
*
*   Error handling
