...
*   store V(0), V(1), ... .                                            *
*                                                                      *
*   Call(s) to: [LIB92]: CONVRT, GETYN, LODRES, OPENFL.                *
*               [GENMCP]: GETINF.                                      *
*                                                                      *
...
*
            WRITE(istde,*) 'Files  mcp  found; enter another root '
     &,                    'file name if these files are not to be'
            WRITE(istde,*) 'used as the GRASP92 MCP Files; '
     &,                    'null otherwise:'
            READ (*,'(A)') FILNAM
*
...
*
         ELSE
*
    3       WRITE(istde,*) 'Enter the root name for the GRASP92 '
     &,                    'MCP Files:'
            READ (*,'(A)') FILNAM
*
...
*
         DO 8 K = 30,32+KMAX
            ...
            IF ((IOS .NE. 0) .OR. (MCPRES .NE. 'MCP')) THEN
               WRITE(istde,*) 'Not a GRASP92 MCP File;'
               DO 7 I = 30,32+KMAX
                  CLOSE (I)
    7          CONTINUE
               GOTO 3
