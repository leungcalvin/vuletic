...
options.disp = 0;options.maxit = 10000;options.tol = 1e-10;
...
figure;subplot(2,1,1);plot(x,V.^2);title('Numerical (Hydrogenlike-ish) eigenfunctions')
...
subplot(2,1,2);title('Analytical (Hydrogenlike) non-relativistic wavefunctions');hold on;
for idx=(l_oam+1):1:(l_oam + nmodes)
    Vsq(:,idx-l_oam) = x.^2.*Rhyd(x,idx,l_oam,netZ).^2.*dx;
end
plot(x,Vsq);
